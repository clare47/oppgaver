...
- [ ] Velg først en litt lys farge. Klikk deretter på Ellipse-verktøyet til
  venstre på skjermen, og tegn en ellipse rundt hodet på katten. Du gjør det ved
  å klikke på pensel, velg sirkel, og slette fyll (rød strek).

  ![Bilde av katt med oksygentank](katt_oksygentank.png)

Til slutt lager vi også en jordklode, som katten kan fly rundt.

- [ ] Klikk på fanen `kode` og deretter på katte-ikonen ![Velg figur fra
  biblioteket](../bilder/hent-fra-bibliotek.png). Velg jordklodefiguren
  `Alle/Earth`. Plasser denne litt på siden av skjermen.
...
- [ ] Av og til vil katten fly bak jordkloden. For å slippe det kan du legge
  klossen `legg foran`{.blocklooks} først i katten sitt skript. Krympet katten
  seg for mye? Du kan endre størrelsen ved å skrive inn 100 i figurvinduet. Da
  blir katten 100% igjen.
